 ...
 *       Q.Safecloud.Client.loadCapability) → Client.stream() → service-worker
 *       HLS through videojs VHS (or Blob fallback) → chunk requests follow
 *       play/pause/seek/buffer via the prefetch loop → each request carries
 *       EIP-712 payment tokens when Client.init() established a payer key.
 *   - state.safecloud — configuration for the adapter (see below)
 *   - startStream(manifest, capability, options) — used by the demo page
 *     and by embed.html to hand content to an already-activated player
 *   - URL scheme 'safecloud:<rootCid>' routes to the adapter
 *
 ...
 *  @param {Object} [options.safecloud] SAFECLOUD: play encrypted content
 *   @param {String} [options.safecloud.rootCid]    content id (for IndexedDB lookup)
 *   @param {Object} [options.safecloud.manifest]   manifest from Client.store()
 *   @param {Object} [options.safecloud.capability] { rootKey } or { grants }
 *   @param {String} [options.safecloud.rootKey]    shortcut for capability.rootKey
 *   @param {String} [options.safecloud.jetUrl]     Jet server URL
 *   @param {Boolean}[options.safecloud.skipPayment] free/public content
 *   @param {Array}  [options.safecloud.payments]   pre-built payment tokens
 *   @param {Number} [options.safecloud.prefetchAhead=3]
 *   @param {Boolean}[options.safecloud.persist=true] save capability to IndexedDB
 ...
 */
var _videoConstructor = function (options) {
	var tool = this;
	...
	// ── SAFECLOUD ADAPTER ─────────────────────────────────────────────────────
	// Plays encrypted Safecloud content. Chunk requests follow playback via
	// Client._prefetchLoop; each request auto-signs payment tokens when the
	// payer key is established (Client.init). Works inside an iframe with keys
	// loaded from IndexedDB — nothing sensitive needs to be in the URL.
	tool.adapters.safecloud = {
		init: function () {
			var cfg = state.safecloud || {};

			// 'safecloud:<rootCid>?jet=<url>' scheme support
			if (!cfg.rootCid && state.url && /^safecloud:/i.test(state.url)) {
				var rest  = state.url.slice('safecloud:'.length).replace(/^\/\//, '');
				var parts = rest.split('?');
				cfg.rootCid = parts[0];
				if (parts[1]) {
					parts[1].split('&').forEach(function (kv) {
						var p = kv.split('=');
						if (p[0] === 'jet') { cfg.jetUrl = decodeURIComponent(p[1] || ''); }
					});
				}
				state.safecloud = cfg;
			}

			if (!Q.Safecloud || !Q.Safecloud.Client) {
				return _fail(new Error('Safecloud plugin not loaded'));
			}
			if (cfg.jetUrl) { Q.Safecloud.Jets.url = cfg.jetUrl; }
			Q.Safecloud.Jets.connect();

			// Resolve manifest + capability: explicit state, else IndexedDB
			var capability = cfg.capability
				|| (cfg.rootKey ? { rootKey: cfg.rootKey } : null);

			var resolved;
			if (cfg.manifest && capability) {
				resolved = Promise.resolve({
					manifest: cfg.manifest, capability: capability
				});
			} else if (cfg.rootCid) {
				resolved = Q.Safecloud.Client.loadCapability(cfg.rootCid)
				.then(function (saved) {
					if (saved && saved.manifest) {
						return {
							manifest:   cfg.manifest || saved.manifest,
							capability: capability   || saved.capability
						};
					}
					return { manifest: cfg.manifest, capability: capability };
				});
			} else {
				resolved = Promise.resolve({ manifest: null, capability: null });
			}

			resolved.then(function (r) {
				if (!r.manifest || !r.capability) {
					return _fail(new Error(
						'Safecloud: no capability for this content on this device'));
				}

				// Persist so refreshes / iframe revisits need no keys in the URL
				if (cfg.persist !== false && r.manifest.rootCid) {
					Q.Safecloud.Client.saveCapability(r.manifest.rootCid, {
						manifest: r.manifest, capability: r.capability
					}).catch(function () {});
				}

				// Establish payer identity silently (no prompt inside an
				// embed); pages wanting the WebAuthn prompt call
				// Q.Safecloud.Client.init({interactive:true}) on a gesture.
				var payerReady = cfg.skipPayment
					? Promise.resolve()
					: Q.Safecloud.Client.init({ interactive: false })
						.catch(function () {});

				return payerReady.then(function () {
					tool.setStatus('Loading…', 'working');

					// Render the player shell first; attach the stream in the
					// ready callback so the prefetch loop can track playback.
					tool._safecloudOnReady = function (player) {
						var videoEl = $("video", tool.element)[0];
						var useSW = ('serviceWorker' in navigator) && !state.isIos;

						Q.Safecloud.Client.stream(r.manifest, r.capability, {
							videoElement:  videoEl,
							setSrc:        false,   // videojs VHS must own the source
							path:          useSW ? 'sw' : 'blob',
							at:            (tool.calculateStartPosition() || 0) / 1000,
							prefetchAhead: cfg.prefetchAhead || 3,
							payments:      cfg.payments,
							skipPayment:   cfg.skipPayment,
							onError: function (err) {
								console.warn('Q/video safecloud:', err);
							}
						}).then(function (handle) {
							tool.safecloudHandle = handle;

							if (handle.path === 'sw') {
								player.src({ src: handle.url,
									type: 'application/x-mpegURL' });
							} else {
								player.src({ src: handle.url,
									type: (r.manifest && r.manifest.type)
										|| 'video/mp4' });
							}

							// Drive the prefetch loop from playback events
							state.onPlay.add(function () {
								handle.resume();
							}, 'Q/video/safecloud');
							state.onPause.add(function () {
								handle.pause();
							}, 'Q/video/safecloud');
							state.onSeek.add(function (position) {
								handle.seek((position || 0) / 1000);
							}, 'Q/video/safecloud');
					...
					tool.initVideojsPlayer({ sources: [] });
				});
			}).catch(_fail);

			function _fail(err) {
				console.warn('Q/video safecloud:', err);
				tool.setStatus(err.message || String(err), 'error');
				Q.handle(state.onError, tool, [err && err.message]);
			}
		}
	};
	// ── END SAFECLOUD ADAPTER ─────────────────────────────────────────────────
...
var _videoDefaults = {
	url: null,
	safecloud: null, // SAFECLOUD: see constructor docblock
	autoplay: false,
	loop: false,
	muted: false,
	controls: true,
...
var _videoMethods = {
	...
	/**
	 * SAFECLOUD: hand content to an already-activated player.
	 * Used by the demo page (after upload) and by embed.html.
	 *
	 * @method startStream
	 * @param {Object} manifest    from Client.store() or a share link
	 * @param {Object} capability  { rootKey } or { grants }
	 * @param {Object} [options]   merged into state.safecloud
	 */
	startStream: function (manifest, capability, options) {
		var tool = this;
		var state = this.state;

		if (tool.safecloudHandle) {
			try { tool.safecloudHandle.stop(); } catch (e) {}
			tool.safecloudHandle = null;
		}
		state.safecloud = Q.extend({}, state.safecloud, options, {
			manifest:   manifest,
			capability: capability,
			rootCid:    (manifest && manifest.rootCid)
				|| (state.safecloud && state.safecloud.rootCid)
		});
		tool.adapters.safecloud.init();
	},
	/**
	 * SAFECLOUD: small status line under the player.
	 * @method setStatus
	 * @param {String} msg
	 * @param {String} [cls]  '' | 'working' | 'ok' | 'error'
	 */
	setStatus: function (msg, cls) {
		var $te = $(this.element);
		var $s  = $te.find('.Q_video_status');
		if (!$s.length) {
			$s = $('<div class="Q_video_status">').appendTo($te);
		}
		$s.text(msg || '')
		  .removeClass('working ok error')
		  .addClass(cls || '')
		  .toggle(!!msg);
	},
	/**
	...
	initVideojsPlayer: function (options) {
		...
		Q.Template.render('Q/video/videojs', fields, function (err, html) {
			...
			state.player = videojs($("video", tool.element)[0], options, function onPlayerReady() {
				...
				// SAFECLOUD: adapter attaches the stream once the player exists
				if (tool._safecloudOnReady) {
					var _scReady = tool._safecloudOnReady;
					tool._safecloudOnReady = null;
					_scReady(player);
				}
	...
	adapterNameFromUrl: function (url) {
		...
		// before any URL parsing (custom schemes don't parse as hosts)
		if (state.safecloud) {
			return 'safecloud';
		}
		if (url && /^safecloud:/i.test(url)) {
			return 'safecloud';
		}
	...
	Q: {
		beforeRemove: function () {
			...
			if (this.safecloudHandle) {
				try { this.safecloudHandle.stop(); } catch (e) {}
				this.safecloudHandle = null;
			}
...
Q.Tool.define("Q/video", _videoConstructor, _videoDefaults, _videoMethods);
Q.Tool.define("Safecloud/video", _videoConstructor, _videoDefaults, _videoMethods);
