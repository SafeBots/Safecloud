...
Jets call `Q.Crypto.OpenClaim.verify(claim)` to check:
...
Jets issue random challenges: "prove you still have CID X by returning the chunk." The Drop returns the chunk and `Drops.verifyChallengeResponse()` recomputes `SHA-256(ciphertext || tag)` to confirm it equals the requested CID. CID mismatch emits `dropChallengeFail` for reputation tracking.
...
## 15. OCP wiring (implemented)

All OCP verification is wired into the production code:

| Location | What it does |
|----------|-------------|
| `Jets.js` `_checkPayments()` | `Q.Crypto.OpenClaim.EVM.verify()` on every payment token, with `ethers.verifyTypedData()` fallback. Enforces time bounds (`nbf`/`exp`), chain allow-list, token allow-list, recipients hash, policy split validation (fractions sum to 10000 bps), and economic admission (Jet must be a payee with `>= minInfraBp`). In signature-only mode (`requirePayment: false`), EIP-712 sig is still the gate; only the on-chain balance pre-screen is skipped. |
| `Jets.js` `verifySubtreeGrant()` | `Q.Crypto.OpenClaim.verify(delegation)` on Drop registration — checks expiry, session-key match, and P-256 signature. Invalid delegation → `Unauthorized` error before the Drop is registered. |
| `Jets.js` chunk/challenge handler | Calls `Drops.verifyChallengeResponse(cid, chunk)` — verifies `SHA-256(ciphertext \|\| tag) == requestedCid`. CID mismatch emits `dropChallengeFail` for reputation tracking. |
| `Drops.js` `verifyAnnounce()` | P-256 signature verification on announce payloads (RFC 8785 canonical JSON, raw r‖s → DER conversion, `crypto.verify`). |
| `Drops.js` `verifyChallengeResponse()` | Content-addressed integrity: recomputes CIDv1 from chunk bytes and compares to the requested CID. |
...
Items 1–4 from the original TODO list are **done** and tested (491 assertions).

| Item | Status |
|------|--------|
| OCP authorization verification in Jets | Done — `Q.Crypto.OpenClaim.verify()` in `verifySubtreeGrant()` |
| OCP payment verification in Jets | Done — `Q.Crypto.OpenClaim.EVM.verify()` + `ethers.verifyTypedData()` fallback in `_checkPayments()` |
| Drop proof-of-storage challenge | Done — `verifyChallengeResponse()` computes CIDv1 from chunk bytes |
| Range request CID routing index | Done — `_cidIndex` with `_leafRangeStart()` range resolution |
| Prolly-backed Drop selection | Wired — `Router.onDropRegistered(drop, prollyRoot)` passes the root; Router uses it for coverage-weighted selection when `Q.Data.Prolly` is available |
| `Q.Data.Merkle`/`Prolly`/`Bloom` integration | Wired — Jets.js calls the canonical `Q.Data.*` methods with fallbacks when the platform primitives are available |
| Jet-to-Jet peering (JetSwarm) | Implemented — Hyperswarm-based with Noise encryption, fan-out fetch, CID-range announcements, grant forwarding. Mode 2 (epoch-based Sybil resistance) is documented but not yet activated |
| Erasure coding | Not yet — replication only. Future work for bandwidth efficiency |
